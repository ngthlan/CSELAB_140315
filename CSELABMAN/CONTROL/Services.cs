...
namespace CSELABMAN.CONTROL
{
            ...
            try
            {
                ...
                if (Data[0].Equals('<') || Data[0].Equals('?') || Data[0].Equals('!') || Data[0].Equals(' ') || Data[0].Equals('\r') || Data[0].Equals('\n')
                 || Data[1].Equals('<') || Data[1].Equals('?') || Data[1].Equals('!') || Data[1].Equals(' ') || Data[1].Equals('\r') || Data[1].Equals('\n'))
                    ...
                    cmpres = false;*/
                tmp1 = appservices.SplitHTML(Data);
    ...
    class AppServices
    {
        ...
        // tach ma HTML
        public string SplitHTML(string data)
        {
            string des = null;
            string[] tmp = null;
        ...
        // doc cac thong tin cau hinh tu file txt CONFIG
        public void readCONF(string dataCONF, ref Label label_Baud, ref ComboBox cmB_Baud, ref Label label_Parity, ref ComboBox cmB_Parity)
        {
            ...
            for (int i = 0; i < tmp.Length; i++)
            {
                if (i == 1)                                            // lay Baudrate
                {
                    countnext = tmp[i].Length - label_Baud.Text.Length - 1;     // lay do dai thong tin Baudrate tuong ung
                    tmpnext = new char[countnext];                          // tao char []                     
                    tmp[i].CopyTo(label_Baud.Text.Length + 1, tmpnext, 0, countnext);
                    cmB_Baud.SelectedItem = chararraytostring(tmpnext);
                    //Console.WriteLine("tmpnext is : " + services.Local.chararraytostring(tmpnext));
                }
                else if (i == 2)                                          // lay Parity
                {
                    countnext = tmp[i].Length - label_Parity.Text.Length - 1;     // lay do dai thong tin Baudrate tuong ung
                    tmpnext = new char[countnext];                          // tao char []                     
                    tmp[i].CopyTo(label_Parity.Text.Length + 1, tmpnext, 0, countnext);
                    cmB_Parity.SelectedItem = chararraytostring(tmpnext);
                    //Console.WriteLine("tmpnext is : " + services.Local.chararraytostring(tmpnext));
                }
            }
        }
